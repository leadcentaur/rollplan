...
export type StatusType = "experimental" | "coming-soon" | "new";
...
const cardGraidents: Record<FeatureTier, string> = {
...
export default function Features({ header, description, features, reversed, tier }: FeatureProps) {
	return (
		<div>
			<div className="relative py-4 sm:py-5 lg:py-6">
				<div className="mx-auto max-w-md px-2 sm:max-w-3xl sm:px-3 lg:max-w-7xl lg:px-4">
					...
					>
						...
						<section className="md:w-2/3 bg-white-500 rounded-lg p-2 md:p-3 lg:p-4 drop-shadow z-10">
								...
								{features.map((feature) => (
									<li key={feature.title} className="flex">
										<div className="pt-1 w-9 shrink-0 grow-0">
											...
											<Icon
												icon={feature.icon}
												className={cx('text-3xl justify-center flex', iconColors[tier])}
												style={{ maxWidth: 54 }}
											/>
